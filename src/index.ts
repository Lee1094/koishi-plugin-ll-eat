...
export const Config: Schema<Config> = Schema.object({
  unsplashKey: Schema.string().role('secret')
    .description('Unsplash Access Key（饮品用）').required(),
...
const FOODS: Dish[] = [
  ...
  { name: '黄焖鸡', meals: ['lunch', 'dinner'], search: '黄焖鸡' },
  { name: '宫保鸡丁', meals: ['lunch', 'dinner'], search: '宫保鸡丁' },
  { name: '鱼香肉丝', meals: ['lunch', 'dinner'], search: '鱼香肉丝' },
  { name: '红烧肉', meals: ['lunch', 'dinner'], search: '红烧肉' },
  { name: '番茄炒蛋', meals: ['lunch', 'dinner'], search: '番茄炒蛋' },
  { name: '土豆烧牛肉', meals: ['lunch', 'dinner'], search: '土豆烧牛肉' },
  { name: '回锅肉', meals: ['lunch', 'dinner'], search: '回锅肉' },
  { name: '糖醋里脊', meals: ['lunch', 'dinner'], search: '糖醋里脊' },
  { name: '麻婆豆腐', meals: ['lunch', 'dinner'], search: '麻婆豆腐' },
  { name: '卤肉饭', meals: ['lunch', 'dinner'], search: '卤肉饭' },
  { name: '咖喱鸡', meals: ['lunch', 'dinner'], search: '咖喱鸡' },
  { name: '黑椒牛柳', meals: ['lunch', 'dinner'], search: '黑椒牛柳' },
  { name: '叉烧', meals: ['lunch', 'dinner'], search: '叉烧' },
  ...
  { name: '鸡腿汉堡', meals: ['lunch', 'dinner', 'night'], search: '鸡腿汉堡' },
  ...
  { name: '薯条', meals: ['lunch', 'dinner', 'night'], search: '薯条' },
  ...
  { name: '大盘鸡', meals: ['lunch', 'dinner'], search: '大盘鸡' },
  { name: '猪肉炖粉条', meals: ['lunch', 'dinner'], search: '猪肉炖粉条' },
  { name: '快餐盒饭', meals: ['lunch', 'dinner'], search: '快餐盒饭' },
  { name: '食堂菜', meals: ['lunch', 'dinner'], search: '食堂菜' },
]
...
interface CachedItem { name: string; imageUrl: string }
...
export function apply(ctx: Context, config: Config) {
  ...
  function filterDishes(list: Dish[]): Dish[] {
    ...
    const f = list.filter(d => d.meals.includes(meal))
    ...
    try {
      ...
      for (const d of fDishes) {
        ...
        if (img) {
          ...
          for (const dd of fDishes.filter(x => x.search === d.search)) foods.push({ name: dd.name, imageUrl: img })
        } else {
          fail++
        }
        await sleep(300) // 请求间隔，避免反爬
      }
      if (foods.length) foodCache = foods
      logger.info(`[${mealName}] 下厨房 成功${ok} 失败${fail} 食物${foodCache.length}条`)
      ...
      for (const d of dDishes) {
        ...
        if (img) {
          ...
          for (const dd of dDishes.filter(x => x.search === d.search)) drinks.push({ name: dd.name, imageUrl: img })
        }
      }
